...
const DEFAULT_INPUTS = {
  minWinrate: '', maxDrawdown: '', minBalance: '', maxBalance: '',
  pageSize: '100', botFilter: 'default', activityFilter: 'all', positionsFilter: 'all',
};
...
const SELECT_FIELDS = [
  { key: 'botFilter',       label: 'Bot Filter',       helper: 'Filter by bot detection',
    options: [['default','All traders'],['no','Humans only'],['yes','Bots only']] },
  { key: 'positionsFilter', label: 'Open Positions',
    options: [['all','All traders'],['yes','In positions'],['no','No positions']] },
  { key: 'activityFilter',  label: 'Activity',          helper: 'Trading volume > 0',
    options: [['all','All traders'],['active','Active only'],['inactive','Inactive only']] },
  { key: 'pageSize',        label: 'Traders Per Page',  helper: 'How many to load at once',
...
export default function ProfitableTradersPage() {
  ...
  useEffect(() => {
    ...
    setInputs({ minWinrate: f.minWinrate ?? '', maxDrawdown: f.maxDrawdown ?? '',
                minBalance: f.minBalance ?? '', maxBalance: f.maxBalance ?? '',
                pageSize: f.pageSize ?? '100', botFilter: f.botFilter ?? 'default',
                activityFilter: f.activityFilter ?? 'all', positionsFilter: f.positionsFilter ?? 'all' });
  ...
  // Debounced search
  useEffect(() => {
    const t = setTimeout(() => setAppliedFilters(prev => ({ ...prev, search: searchQuery })), 400);
  ...
  const handleApplyFilters = () => {
    setAppliedFilters(toApplied({ ...inputs, search: searchQuery }));
    setPageSize(Math.min(Math.max(parseInt(inputs.pageSize) || 100, 10), 200));
  };
  ...
  const hasUnappliedChanges = useMemo(() => {
    ...
    return JSON.stringify(current) !== JSON.stringify(appliedFilters)
      || (parseInt(inputs.pageSize) || 100) !== pageSize;
  }, [inputs, searchQuery, appliedFilters, pageSize]);

  const handleWalletClick = (wallet) => {
    sessionStorage.setItem('traderFilters', JSON.stringify({ ...inputs, sortBy, sortDirection, searchQuery }));
    navigate(`/trader/${wallet}`);
  };

  const addToWatchlist = (wallet) => {
    if (!userId) return;
    fetch('http://localhost:8000/api/watchlist', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ user_id: userId, wallet_address: wallet, label: wallet }),
    }).then(res => alert(res.status === 409 ? 'Already in watchlist' : 'Added to watchlist!'));
  };
  ...
  const stats = useMemo(() => {
    ...
    return {
      loaded: traders.length, total: pagination.total_count,
      displayed: traders.length, profitable,
      avgGain:    traders.length ? totalGain / traders.length : 0,
      avgWinrate: traders.length ? traders.reduce((s, t) => s + (t.winrate || 0), 0) / traders.length : 0,
    };
  }, [traders, pagination]);
  ...
  if (initialLoading) return (
    <div className={styles.discordContainer}>
      <div className={styles.loadingState}>
        <div className={styles.spinner} /><p>Loading traders...</p>
      </div>
    </div>
  );
        ...
        <div className={styles.filterSection}>
          {FILTER_FIELDS.map(({ key, label, placeholder, helper, ...rest }) => (
            <label key={key} className={styles.filterLabel}>
              ...
              <input
                type="number" placeholder={placeholder} value={inputs[key]}
                onChange={set(key)} className={styles.discordInput}
          ...
          {SELECT_FIELDS.map(({ key, label, helper, options }) => (
            <label key={key} className={styles.filterLabel}>
              <span className={styles.labelText}>{label}</span>
              <select value={inputs[key]} onChange={set(key)} className={styles.discordSelect}>
          ...
          <div className={styles.filterActions}>
            ...
            <button onClick={handleClearFilters} className={styles.clearButton}>Clear All</button>
          </div>
        </div>
      ...
      <div className={styles.mainContent}>
        <div className={styles.channelHeader}>
          ...
          <div className={styles.searchBar}>
            <input type="text" placeholder="Search wallets..." value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)} className={styles.searchInput} />
          </div>
        </div>

        <TraderTable
          traders={traders} sortBy={sortBy} sortDirection={sortDirection}
          handleSort={handleSort} onWalletClick={handleWalletClick}
          actionLabel="Watch" onAction={addToWatchlist} actionIcon="★"
          actionColor="#5865f2" styles={styles}
          footer={
            <>
              {hasMore && (
