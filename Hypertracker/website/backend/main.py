...
async def get_profitable_traders(
    ...
    activity_filter: str = Query("all"),
    positions_filter: str = Query("all"),
    ...
    if activity_filter == "active":
        query["total_volume_usdc"] = {"$gt": 0}
    elif activity_filter == "inactive":
        query["total_volume_usdc"] = {"$lte": 0}
    if positions_filter == "yes":
        query["open_positions_count"] = {"$gt": 0}
    elif positions_filter == "no":
        query["open_positions_count"] = 0
    if is_bot == "true":
