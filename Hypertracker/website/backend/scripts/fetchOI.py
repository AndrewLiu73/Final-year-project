...
async def fetch_bybit_oi(client, coin):
    symbol    = f"{coin}USDT"
    oi_url    = ("https://api.bybit.com/v5/market/open-interest?category=linear&symbol={symbol}&intervalTime=5min&limit=1")
    price_url = ("https://api.bybit.com/v5/market/tickers?category=linear&symbol={symbol}")
...
async def fetch_deribit_oi(client, coin):
    url = ("https://www.deribit.com/api/v2/public/get_book_summary_by_currency?currency={coin}&kind=future")
